    ...
    <div className="relative">
      ...
      <div className="absolute top-8 left-2">
        <svg
          ...
          strokeWidth={1.5}
          stroke="currentColor"
          className={`w-5 h-5 stroke-current ${newMessage ? 'text-red-700' : 'text-gray-400'}`}
        >
          <path strokeLinecap="round" strokeLinejoin="round" d={icons.find((i) => i.title === id)?.path} />
        </svg>
      </div>
      <input
        className={`appearance-none block w-full px-3 py-2 pl-8 border h-10 ${
        ...
        } focus:${newMessage ? 'border-red-700' : 'border-gray-300'} sm:text-sm ${
      ...
      <p className="block text-sm font-medium text-red-700 mt-1 h-4 ml-1">{newMessage}</p>
    </div>
  );
}
